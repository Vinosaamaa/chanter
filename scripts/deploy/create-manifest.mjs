...
import { recoveryCapability } from './recovery-runtime.mjs';
...
const recovery = recoveryCapability(JSON.parse(fs.readFileSync('infra/production/recovery-policy.json', 'utf8')));
...
fs.writeFileSync(output, JSON.stringify(validateRelease({ version: 1, commit, architecture, schemaEpoch: policy.schemaEpoch, images, ...recovery }), null, 2) + '\n');
