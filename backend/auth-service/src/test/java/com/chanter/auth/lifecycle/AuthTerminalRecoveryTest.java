...
class AuthTerminalRecoveryTest {
    ...
    @Test void actualAccountEffectsCanonicalImportAndReceiptCommitTogetherThenAllRestoredCredentialsClose() {
        ...
        assertThat(receipt).isEqualTo(new TerminalReapplyStore.Receipt(1,"auth",head,0,1));
        ...
        assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM auth_email_tokens WHERE id=?",Integer.class,emailToken)).isZero();
        assertThat(jdbc.queryForObject("SELECT email FROM auth_users WHERE id=?",String.class,account.user().id())).isEqualTo("deleted:"+account.user().id());
