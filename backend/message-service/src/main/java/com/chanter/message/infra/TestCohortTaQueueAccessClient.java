...
public class TestCohortTaQueueAccessClient implements CohortTaQueueAccessClient {
    ...
    public void registerCohort(UUID cohortId, UUID courseId) {
        existingCohorts.add(cohortId);
        accessRules.putIfAbsent(
                key(cohortId, UUID.randomUUID()),
                new CohortTaQueueAccess(cohortId, courseId, false, false)
        );
    }

    public void grantLearnerAdd(UUID cohortId, UUID userId, UUID courseId) {
        existingCohorts.add(cohortId);
        accessRules.put(key(cohortId, userId), new CohortTaQueueAccess(cohortId, courseId, true, false));
    }

    public void grantInstructorManage(UUID cohortId, UUID userId, UUID courseId) {
        existingCohorts.add(cohortId);
        accessRules.put(key(cohortId, userId), new CohortTaQueueAccess(cohortId, courseId, false, true));
    }
    ...
    public CohortTaQueueAccess requireAccess(UUID cohortId, UUID userId) {
        ...
        throw new ResponseStatusException(HttpStatus.FORBIDDEN, "TA Queue access requires Cohort Enrollment or Instructor role");
    }
