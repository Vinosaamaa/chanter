...
public final class ExportRequestController {
    ...
    public ExportRequestController(ExportParticipant participant, AccountExportProtocol protocol, @Value("${chanter.internal-service-token}") String token) {
        this.participant = participant; this.protocol = protocol; this.access = new InternalLifecycleAccess(token);
    }
    ...
    public ResponseEntity<Void> accept(@RequestBody String body,
        ...
        try { participant.accept(protocol.event(body)); }
