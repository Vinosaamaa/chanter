...
public final class DeletedScopeStore {
    public static final String SCHEMA="""
        ...
        """;
    public static final String RECOVERY_SCHEMA=SCHEMA.replace("lifecycle_scope_import","lifecycle_recovery_scope");
    ...
    public DeletedScopeStore(JdbcTemplate jdbc,TransactionTemplate tx,Consumer<TerminalJournal.Entry> authority,
        ...
        this(jdbc,tx,authority,reconcile,false);
    }
    DeletedScopeStore(JdbcTemplate jdbc,TransactionTemplate tx,Consumer<TerminalJournal.Entry> authority,
            Consumer<TerminalJournal.Entry> reconcile,boolean recovery) {
        this.jdbc=jdbc; this.tx=tx; this.authority=authority; this.reconcile=reconcile; this.recovery=recovery;
    }

    public DeletedScope.Receipt accept(DeletedScope.Import request) {
        request.validate();
        if(recovery) throw new IllegalStateException("Recovery scope requires its verified restore binding");
        return accept(request,request.page().terminalDigest());
    }

    DeletedScope.Receipt accept(DeletedScope.Import request,String basis) {
        request.validate();
        return tx.execute(status -> {
            ...
            if(rows.isEmpty()) {
                ...
                header=new Header(page.terminalRevision(),page.terminalEventId(),page.terminalDigest(),page.totalCount(),page.scopeDigest(),
                        0,DeletedScope.START,DeletedScope.startDigest(basis,page.kind(),page.totalCount()),false,basis);
                jdbc.update(sql("INSERT INTO lifecycle_scope_imports VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"),page.studyServerId(),page.kind(),
                        header.revision(),header.event(),header.terminalDigest(),header.total(),header.digest(),0,header.after(),header.rolling(),false,basis);
            } else header=rows.getFirst();
            match(header,page);
            if(!header.basis().equals(basis)) throw new IllegalArgumentException("Scope restore binding changed");
            var accepted=jdbc.query(sql("SELECT page_digest FROM lifecycle_scope_import_pages WHERE study_server_id=? AND scope_kind=? AND after_id=?"),
            ...
            for(UUID id:page.ids()) jdbc.update(sql("INSERT INTO lifecycle_scope_import_ids VALUES (?,?,?)"),page.studyServerId(),page.kind(),id);
            UUID after=page.ids().isEmpty() ? page.after() : page.ids().getLast();
            jdbc.update(sql("UPDATE lifecycle_scope_imports SET received_count=?,after_id=?,rolling_digest=?,ready=? WHERE study_server_id=? AND scope_kind=?"),
                    count,after,rolling,ready,page.studyServerId(),page.kind());
            jdbc.update(sql("INSERT INTO lifecycle_scope_import_pages VALUES (?,?,?,?)"),page.studyServerId(),page.kind(),page.after(),page.pageDigest());
            if(ready) reconcile.accept(request.entry());
            return receipt(page,new Header(header.revision(),header.event(),header.terminalDigest(),header.total(),header.digest(),count,after,rolling,ready,basis));
        });
    }
    ...
    public String scopeDigest(TerminalJournal.Entry entry,String kind) {
        if(!ready(entry,kind)) throw new IllegalArgumentException("Scope is not READY");
        return headers(entry.targetId(),kind).getFirst().digest();
    }
    public DeletedScope.Receipt receipt(TerminalJournal.Entry entry,String kind) {
        if(!ready(entry,kind)) throw new IllegalArgumentException("Scope is not READY");
        var header=headers(entry.targetId(),kind).getFirst();
        return new DeletedScope.Receipt(1,entry.targetId(),entry.revision(),entry.eventId(),entry.digest(),kind,
                header.total(),header.digest(),header.received(),header.after(),header.ready());
    }
    String basis(TerminalJournal.Entry entry,String kind) {
        if(!ready(entry,kind)) throw new IllegalArgumentException("Scope is not READY");
        return headers(entry.targetId(),kind).getFirst().basis();
    }
    ...
    public DeletedScope.Page page(UUID server,long revision,UUID event,String digest,String kind,UUID after,int limit) {
        ...
        var ids=jdbc.query(sql("SELECT scope_id FROM lifecycle_scope_import_ids WHERE study_server_id=? AND scope_kind=? AND scope_id>? ORDER BY scope_id LIMIT ?"),
    ...
    private List<Header> headers(UUID server,String kind) {
        return jdbc.query(sql("SELECT revision,event_id,terminal_digest,total_count,scope_digest,received_count,after_id,rolling_digest,ready,basis_digest FROM lifecycle_scope_imports WHERE study_server_id=? AND scope_kind=?"),
                (rs,row)->new Header(rs.getLong(1),rs.getObject(2,UUID.class),rs.getString(3),rs.getLong(4),rs.getString(5),rs.getLong(6),
                        rs.getObject(7,UUID.class),rs.getString(8),rs.getBoolean(9),rs.getString(10)),server,kind);
    }
    ...
    private String sql(String text) { return recovery ? text.replace("lifecycle_scope_import","lifecycle_recovery_scope") : text; }
    private record Header(long revision,UUID event,String terminalDigest,long total,String digest,long received,UUID after,String rolling,boolean ready,String basis) { }
}
