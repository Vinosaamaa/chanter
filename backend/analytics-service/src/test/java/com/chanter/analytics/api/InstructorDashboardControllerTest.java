...
class InstructorDashboardControllerTest {
    ...
    void instructorCanLoadDashboardAggregates() throws Exception {
        ...
        UUID questionChannelId = UUID.randomUUID();
        ...
        when(instructorDashboardService.buildDashboard(eq(studyServerId), eq(viewerUserId)))
                .thenReturn(new InstructorDashboardResponse(
                        ...
                        false,
                        3,
                        List.of(new TeachingCourseResponse(
                                courseId,
                                "CS 101",
                                questionChannelId,
                                List.of(new TeachingCohortResponse(cohortId, "Spring 2026", 1)),
                                4,
                                2,
                                5,
                                1
        ...
        mockMvc.perform(get("/api/v1/study-servers/{studyServerId}/instructor-dashboard", studyServerId)
                ...
                .andExpect(jsonPath("$.courses[0].questionChannelId").value(questionChannelId.toString()))
                .andExpect(jsonPath("$.courses[0].cohorts[0].cohortId").value(cohortId.toString()));
    }
}
