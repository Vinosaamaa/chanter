...
export function ConversationPlaceholder() {
  ...
  if (!channelScope || !channelId) {
    return (
      <ConversationFrame
        ...
        body="Pick a study server channel or a course channel from the sidebar."
      />
    )
  }
  ...
  if (navigationQuery.isLoading) {
    return (
      <ConversationFrame
        ...
        body="Checking your access to this channel."
      />
    )
  }
  ...
  if (navigationQuery.isError) {
    return (
      <ConversationFrame
        title="Study server unavailable"
        body="You do not have access to this study server or it no longer exists."
      />
    )
  }
  ...
  if (!channelLabel) {
    return (
      <ConversationFrame
        ...
        body="This channel was not found or you do not have permission to open it."
      />
    )
  }
  ...
  return (
    <ConversationFrame
      title={channelLabel}
      body={`Messages for ${channelLabel} will appear here once realtime chat ships.`}
    />
  )
}

function ConversationFrame({ title, body }: { title: string; body: string }) {
  return (
    <section className="flex min-w-0 flex-1 flex-col bg-app-bg">
      ...
      <div className="flex flex-1 items-center justify-center p-6">
        <p className="max-w-md text-center text-sm text-app-muted">{body}</p>
      </div>
    </section>
  )
}
