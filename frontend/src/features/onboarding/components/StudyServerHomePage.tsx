    ...
    <section className="flex min-w-0 flex-1 flex-col overflow-y-auto bg-app-bg">
      ...
      <div className="flex-1 space-y-6 p-6">
        {navigationQuery.isLoading && (
          <p className="text-sm text-app-muted">Loading courses…</p>
        )}
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-3 text-xs text-app-muted">
                    {course.channels.length} course channel
                    {course.channels.length === 1 ? '' : 's'}
                  </p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    {firstChannel ? (
                      <Link
                        to={courseChannelPath(serverId, firstChannel.id)}
                    ...
                    {canManage && course.cohorts[0] ? (
                      <Link
                        ...
                        className={cn(
                          'rounded-lg bg-app-accent px-3 py-1.5 text-sm font-medium text-white hover:opacity-90',
                        )}
                      >
                        Manage enrollment
                      </Link>
