...
export function V2CommunityHubLayout() {
  ...
  const context: V2CommunityContext = { serverId, serverName, isOwner: navigationQuery.data?.canViewFullCatalog ?? false, navigation: navigationQuery.data }
  return <section className="v2-workspace-page community-hub-page"><header className="community-hub-chrome"><div className="community-hub-title"><span><Sprout /></span><div><h1>{serverName}</h1><p>Community · 214 members · {navigationQuery.data?.courses.length ?? 3} courses</p><button type="button" className="v2-primary-button"><UserPlus />Invite people</button></div><div className="community-member-stack">{['Sam','Priya','Jordan','Maria'].map((name,index) => <V2Avatar key={name} name={name} tone={index % 2 ? 'purple' : 'amber'} size="lg" />)}<i>+11</i></div></div><nav className="community-tabs">{tabs.map((tab) => <NavLink key={tab.id} to={v2CommunityPath(serverId,tab.id)}>{tab.label}</NavLink>)}</nav></header><div className="community-tab-panel"><Outlet context={context} /></div></section>
}
