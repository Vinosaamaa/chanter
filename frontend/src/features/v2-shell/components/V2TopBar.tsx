...
export function V2TopBar({ onOpenMenu }: V2TopBarProps) {
  ...
  const pageTitle = pathname.startsWith('/app/deletions/') ? 'Deletion status' : pathname.startsWith('/app/account-data') ? 'Account data' : pathname.startsWith('/app/settings') ? 'Settings' : primary ? primary[0].toUpperCase() + primary.slice(1) : 'Home'
