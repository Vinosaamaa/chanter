...
for (const state of ['BUILDING', 'CANCELLED', 'EXPIRED'] as const) {
  test(`account data ${state.toLowerCase()} on phone @accountdata`, async ({ page }, info) => {
    ...
    const outcome = state === 'BUILDING' ? page.getByText(/sources prepared\./)
      : state === 'CANCELLED' ? page.getByText(/Some sources have not yet confirmed removal/)
        : page.getByText(/This export is no longer available/)
