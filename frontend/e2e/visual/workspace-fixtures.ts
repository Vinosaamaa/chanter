...
export const VISUAL_NOW = '2026-09-12T14:00:00Z'
...
export function workspaceFixture(path: string, empty: boolean, staff: boolean, completedNotifications: Set<string>): unknown {
  if (path.endsWith('/search')) return { results: empty ? [] : [
    { documentType: 'RESOURCE', courseId, courseTitle, sourceId: 'visual-resource', title: 'Problem-solving field notes', snippet: 'A study guide to breaking larger problems into smaller steps.' },
    { documentType: 'FAQ', courseId, courseTitle, sourceId: 'visual-faq', title: 'How should I start a study session?', snippet: 'Write down the question you want to answer before choosing an example.' },
    { documentType: 'MESSAGE', courseId, courseTitle, sourceId: 'visual-message', title: 'Channel message', snippet: 'Would anyone like to compare study notes before Office Hours?', href: `${course}/chat?channel=visual-general` },
    { documentType: 'EVENT', courseId: null, courseTitle: 'Open Learning Collective', sourceId: event.id, title: event.title, snippet: event.description, href: `${community}/events` },
    { documentType: 'ANNOUNCEMENT', courseId: null, courseTitle: 'Open Learning Collective', sourceId: 'visual-announcement', title: 'A fresh starting point for this week', snippet: notification.bodyPreview, href: `${community}/announcements` },
  ] }
  if (path.endsWith('/auth/sessions')) return { sessions: [{ id: 'visual-session-current', createdAt: earlier, lastUsedAt: earlier, expiresAt: later, userAgent: 'Chrome/140 Windows', current: true }, { id: 'visual-session-other', createdAt: earlier, lastUsedAt: earlier, expiresAt: later, userAgent: 'Safari/18 iPhone', current: false }] }
  ...
  if (path.endsWith('/notifications/visual-notification/done')) { completedNotifications.add('visual-notification'); return { ...notification, doneAt: VISUAL_NOW } }
  if (path.endsWith('/notifications/visual-notification/read')) return { ...notification, unread: false, readAt: VISUAL_NOW }
  if (path.endsWith('/notifications')) return { notifications: empty ? [] : [notification, { ...notification, id: 'visual-notification-2', filterBucket: 'MENTIONS', title: 'Alexandra mentioned you in a question', bodyPreview: 'Your sketch helped clarify the first step. How did you decide what to leave out?', unread: false }].filter(item => !completedNotifications.has(item.id)) }
  ...
  if (path.endsWith('/course-catalog')) return { courses: empty ? [] : [courseTitle, 'Designing for people', 'Mathematics for everyday decisions', 'Writing with clarity'].map((title, index) => ({ id: `visual-course-${index}`, title, instructorUserId: instructor, cohorts: [{ id: cohortId, name: 'Autumn cohort', enrollmentPolicy: 'OPEN', enrolled: index === 0, learnerCount: 28 }] })) }
