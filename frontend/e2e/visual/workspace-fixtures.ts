...
export function workspaceFixture(path: string, empty: boolean, staff: boolean, completedNotifications: Set<string>): unknown {
  ...
  if (path.endsWith('/notifications/visual-notification/done')) { completedNotifications.add('visual-notification'); return { ...notification, doneAt: new Date().toISOString() } }
  if (path.endsWith('/notifications/visual-notification/read')) return { ...notification, unread: false, readAt: new Date().toISOString() }
