...
| Browser journey asserted cookie removal before remote logout completed | Await the actual logout HTTP 204 and response completion before reloading and checking cookies. Immediate local logout remains intentional; the durable marker already prevents restoration after interrupted logout. | Full hosted product journey, with local ordering and failed-logout regressions retained. |
