# Disposable source integration preview
...
definitive-403 regression is retained. The preview adds the source-owner-approved
maintenance checks to reserve, claim and migration dispatch, preserving terminal
then budget lock order. Unexpected source text or conflicts stop the preview.
The same union must later be reviewed and accepted in the actual feature diff.

The hosted smoke stack runs the source-owned unshipped helper using its actual
packaged Spring application and normal private permission clients. The setup
