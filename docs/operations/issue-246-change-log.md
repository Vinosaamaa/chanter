...
The rebased head passed complete hosted CI, signed-in product journeys, both native storage/scanner architectures, packaged security and both native release staging jobs. Completed full review then prompted broader permanent scope-status coverage and seed fail-fast handling. Real HTTP and worker tests distinguish permanent client errors from 408/429/server outages. Deployment review clarified that an empty historical deployment remains valid; persistent migration-floor recovery belongs to the deployment owner's #252 change. Final gates run again after these review fixes.
