...
An indexing request may outlive its caller. The agent deletion fence rejects late chunk replacement once deletion commits; media reservations remain held until that response. Live authorization independently rejects stale evidence. See the AI authorization and terminal deletion contract below.
...
1. Put resource uploads and AI resource retrieval behind maintenance before changing storage. Take an encrypted database backup and preserve the original local resource directory. Clear legacy AI resource chunks using authenticated `POST /api/v1/internal/resource-chunks/{resourceId}/purge` before allowing retrieval again; never use terminal `DELETE` for migration because it permanently retires the ID. Old chunks predate the quarantine guarantee.
2. Apply media Flyway V2 and agent Flyway V8 through the reviewed release migration command. Existing rows become `LEGACY`, retain their byte reservation and become unavailable for downloads. Confirm their sum fits the 8 GB resource allocation. Never rewrite V1 or mark legacy rows available manually.
...
Run local module tests with `mvn -s backend/.mvn/settings.xml -f backend/pom.xml -pl agent-service,media-service -am verify`. The isolated real-process suite is `.github/workflows/media-security.yml`: pinned PostgreSQL, Adobe S3Mock and ClamAV images on native AMD64 and ARM64, real EICAR rejection, immutable writes, metered attempts, concurrent PostgreSQL indexing/deletion and permanent deletion-marker verification after preserved-volume process restart. EICAR is the harmless standard antivirus test fixture.
...
## AI authorization and terminal deletion

The agent catalog accepts only current `AVAILABLE`, AI-approved resources in the requested course, authorized by media for the actual viewer. A supplied grant or stale index row alone cannot authorize vector results, chunk tools or answer citations. Processing, failed, rejected, missing and deleted resources fail closed, including instructor access and media outages. The internal vector request includes both course and viewer identifiers.

Terminal `DELETE /api/v1/internal/resource-chunks/{resourceId}` commits a durable agent-owned deletion marker and removes chunks plus cascading embeddings. A late ingestion cannot overwrite that marker. In-progress chunk replacement and deletion serialize on one local database row; media keeps the reservation until the committed deletion response. The separate migration purge route clears content without retiring a live resource identity and cannot undo terminal deletion. Preserve the agent deletion markers in coordinated backups; do not truncate them or restore an older agent database independently while media is serving. Markers retain no source text, but their historical metadata count grows with deleted resource IDs.
