...
An indexing request can continue at the agent service after its caller times out and create chunks after deletion. The media lease protects metadata completion, not remote chunk writes. Before production, retrieval must authorize each resource as currently `AVAILABLE` and approved, and #246/#251 must coordinate durable indexing with deletion. Do not enable production resource AI access based only on a successful cleanup request.
