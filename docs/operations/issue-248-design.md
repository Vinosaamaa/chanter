...
Only complete is authoritative. A container timeout or broken transport can close the stream without an error event: Spring marks the emitter complete before invoking its timeout callback. Clients must treat EOF without complete as interruption and discard partial text/citations, even if valid quote chunks arrived. Error events are best effort while the transport remains writable. A repeated request may recover a persisted answer or report GENERATION_ALREADY_ATTEMPTED; it must not promise paid regeneration.
...
Resource-download and citation-recheck loops check cancellation before the next call. The existing synchronous content request may finish its configured timeout (10 seconds by default) before the loop observes cancellation; this layer does not forcibly interrupt that legacy client. Provider transport has active cancellation hooks. Reauthorization receives the active model deadline, so its citation loop does not continue under the longer outer request budget after model cancellation.
