...
| Critical | `AiQuotaEnforcementService.java` | `pg_advisory_xact_lock` uses single-`bigint` overload with XOR-derived key from study server id |
| Major | `AiQuotaEnforcementService.java` | Only cache successful PostgreSQL detection; retry on transient connection failures |
