...
Migration containers explicitly clear native issuer settings inherited from the agent environment file. A focused Compose-generation test verifies the override and the unchanged runtime values. An inherited #253 gateway test now separates virtual-time expiry from real capacity cancellation, removing a 40 ms CI timing race without changing production behavior.
