...
`media-seed` reserves a STAGING row without writing bytes. It must remain unsettled and cannot prove physical deletion or object restore. #342 owns the separate real object fixture. Historical restored-only child rows and a server created after the selected backup must be exercised by the full native orchestration; this helper's first checkpoint does not claim those journeys passed.
