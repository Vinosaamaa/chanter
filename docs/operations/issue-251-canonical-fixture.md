...
`scripts/deploy/fixtures/CanonicalLifecycleFixture.java` is unshipped hosted test code. Compile it with the packaged service classes/dependencies, then invoke its fixed class name with no arguments. Set `CHANTER_CANONICAL_LIFECYCLE_FIXTURE=true` only in the isolated fixture. The packaged `/app/main-class` selects one of the seven owning services; the helper rejects unknown services and recovery mode. One bounded JSON request arrives on stdin, one JSON result goes to stdout, and application logs go to stderr. Input is limited to 64 KiB and output to 1 MiB. It exposes no HTTP route.
...
| events | any of the seven | afterRevision nonnegative integer | up to 64 original lifecycle destination/event pairs, ordered by revision |
| deliver | any of the seven | original event object | eventId and committed=true after the real owning consumer returns |
...
The source advances at most 256 retained identities per existing outbox command. Exact content deletion removes matching ordinary search/notification payloads and changes those source outbox rows to `ERASED`, which cannot be reclaimed or manually replayed. Shared QUESTION_PREVIEW rows instead keep notification delivery with neutral title/preview/label. New shared updates enter the outbox in that neutral shape. Old retained shared payloads are sanitized through the source closure's immutable revision cutoff, at most 64 rows for one question per advance; unfinished sanitation continues on the same outbox. Already-claimed copies remain covered by permanent recipient markers or shared-preview normalization. No additional retry queue or scheduler is introduced.
...
The community-only community-history-remove-course action accepts exact serverId/courseId/ownerId. It locks terminal authority, requires the current open synthetic server and matching immutable course parent/instructor, bounds channel IDs to 64, then removes that synthetic course graph transactionally. It returns serverId/courseId/channelIds and historicalFixtureOnly=true. This creates the older-backup-only child case; it is not a product deletion path or journal allocation. Back up first, invoke this action, then use the actual server-delete and actual outbox/auth path to allocate the current terminal entry and scope. Wrong parent/owner/nonfixture graph is rejected. The compiled owning-source test verifies rejection, parent-bound removal and ordinary server deletion still requiring canonical delivery.
