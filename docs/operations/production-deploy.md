...
Issue #249 advances the release compatibility epoch to 9. Do not roll back to epoch 8 after its moderation migrations: older binaries do not enforce current account and source restrictions. The frontend proxy checks every `/livekit/*` request with community-service before stripping the prefix and forwarding to private LiveKit signaling. A dedicated internal `media-authorization` network connects only these two services; the proxy retains its existing edge address and never joins the application network. The guard verifies the query join token without a privileged service credential, and failure denies signaling. The proxy bounds guard connection and response-header waits to two and five seconds. Active-participant reconciliation independently removes denied participants.

Operator enrollment additionally requires `CHANTER_OPERATOR_ENCRYPTION_KEY` in the private `auth-service.env`: a distinct canonical base64-encoded 32-byte key, retained with encrypted configuration backups. It is optional for ordinary startup and is neither generated nor copied into other services. Configure it and run the explicit first-operator bootstrap before claiming operator readiness; changing it without retaining the old factors prevents their decryption. Runtime preflight rejects malformed or reused shared credentials. Release smoke validates the actual packaged Caddy adaptation and admits only the 60-second health-room token with publishing, subscription and data disabled. This synthetic probe does not establish a real operator account or prove provider setup.
