...
The native workflow exercises the packaged agent at its 640 MiB allocation and shares two CPUs with the 896 MiB PostgreSQL fixture. It checks real paraphrase relevance separately from a generated 100,000-chunk load corpus, reports p50/p95 inference/database latency and peak container memory, and preserves the actual scoped query plan. The load corpus repeats ten semantic vectors across 1,000 resources and ten courses; it establishes bounded query behavior, not broad educational-answer quality. The change log records completed measurements on both architectures. The enforced gates are the 640 MiB allocation and two-second retrieval p95; observed faster results are not separate hard limits. Final production union testing includes enabled telemetry.
...
Before upgrading to epoch 8, remove CHANTER_GROUNDING_ENGINE=keyword and CHANTER_EMBEDDINGS_PROVIDER=hashing/ollama from existing configuration. The supported resource-grounding mode is rag; external semantic engines use the explicit API adapter. The native migration/index proof uses a non-superuser schema owner, matching the production service's existing ownership privileges.

On the ten fixed paraphrases, the pinned model ranked the correct source first in all ten. Eight exceeded the unchanged 0.35 evidence threshold; illness-on-exam and FIFO wording remained below it and require handoff. The unrelated football question scored at most 0.143. These fixture results separate ranking from supported-answer coverage; they do not establish broad production accuracy.
