...
The Usage page derives access from existing server ownership and navigation capabilities. It does not display usage until the selected server's dashboard request finishes successfully. Loading and transport failures cannot become a fake zero; access-loading failures remain visible instead of masquerading as lost permission. Old billing links redirect to Usage. The dashboard and development harness contain no plan-changing controls.
