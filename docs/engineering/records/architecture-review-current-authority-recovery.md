...
Current Study Server deletions also retain both course and channel scope attachments inside the encrypted journal archive. The independently tested Java/Node count and ordered-ID digest distinguish missing scope from a verified empty set. A separate restore-bound historical union can add source-verified children from the older database while preserving every original archived ID and leaving its digest untouched. Current and derived imports have explicit ordering and receipt checks; actual all-source PostgreSQL recovery remains an integration gate. Private resource byte evidence separately verifies immutable key, namespace, exact size and SHA-256 through full encrypted read-back. Its receipt does not establish a complete source inventory, settled remote writes or provider retention.
