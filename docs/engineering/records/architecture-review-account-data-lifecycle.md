...
Media metadata disposition is separate from physical evidence. It only follows the source state established by the existing verified maintenance hook or worker, plus exact agent acknowledgement. The original physical tuple remains unchanged for #342 inventory checks. The account reverse lookup joins retained content identity to the original terminal revision/event/digest and has its own reverse index. Source-local rollback and stale claimed-delivery tests pass. The export deadline fixture now shares a fixed whole-second persistent clock while retaining real cancellation deadlines; it no longer mixes database and host availability timestamps. Privacy handoff distinguishes source versus overall completion, shared/accounting/recovery preservation, conditional backup expiry and unconfigured provider/contact gaps.

Server accounting unlink uses the same ledger-row serialization and nullable-attribution settlement predicate as account cleanup. Its immutable local disposition count survives native-row removal and repeated cleanup. Both source and derived scopes remain mandatory before completion; acknowledgment follows recorded server/channel identity, without changing canonical authority. Full common/agent verification passed.

Community/message server cleanup now retires queued and claimed search/notification content under exact retained identities or verified server/course/channel scope. The same transaction removes the source graph and office-hours dependencies; rollback retains both payload and graph. Community ordinary completion waits for both scope kinds from all five destinations; message waits for its verified current/derived scope and payload erasure. Full common/community/message tests pass, including 257-row paging, malformed-payload rollback, late delivery callbacks, unrelated content preservation, empty scope acknowledgements and actual migrated graph cleanup. Global completion and physical/provider closure still require the final hosted union.

Release policy now stamps epoch 10 for auth V8/community V19/message V12/media V5/agent V14/notification V3/search V3. Older binaries omit permanent terminal checks and nullable-attribution semantics, so the same-epoch rollback rule rejects epoch 9 before deployment actions. Focused release/recovery policy tests pass. Consistent journal/scope/cursor/object restore and final generated-runtime checks remain part of the combined release; dependent media V6 proof is not inferred from this source policy.

Full-review disposition confirms existing transaction completion and durable consumer identity prevent duplicate export capture; user/session locks serialize cancellation with download issuance; denied ingestion candidates become permanent DELETED rows; expired export history is bounded by the existing expiry worker. The confirmed auth outbox index omission is corrected in unshipped V8 using the established delivery/lease indexes. Migrated account/source deletion tests pass. Broader batching suggestions are deferred; they do not change the final source trust boundary.
