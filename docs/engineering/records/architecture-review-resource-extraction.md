...
Matching READY source bytes, filename and parser version reuse the same chunks without recomputation. Failures retain only a safe FAILED state; they do not publish partial vectors, provider diagnostics or source text. A retry starts a new generation. Process loss leaves PROCESSING for the durable caller to retry; this draft does not introduce a second event/outbox system. Migration purge increments the generation and clears content without retiring the ID. Terminal deletion clears all source/course/filename metadata and retains only the ID, terminal state and generation required to reject late work.
...
V9 introduces generation and extraction metadata. Older writers do not honor the generation boundary, so this change requires a deployment epoch after accepted issue #245 (planned epoch 6). Downgrade to pre-246 application code against this schema is unsupported. No production configuration is changed here.
